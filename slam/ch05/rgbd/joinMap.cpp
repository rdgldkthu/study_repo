...
int main(int argc, char **argv) {
  ...
  if (!fin) {
    cerr << "Please run the program in the directory containing pose.txt" << endl;
    return 1;
  }
  ...
  // Compute the pointcloud using camera intrinsics
  double cx = 325.5;
  double cy = 253.5;
  double fx = 518.0;
  double fy = 519.0;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    for (int v = 0; v < color.rows; v++)
      for (int u = 0; u < color.cols; u++) {
        unsigned int d = depth.ptr<unsigned short>(v)[u]; // 16-bit depth value
        if (d == 0)
  ...
  cout << "Global pointcloud has " << pointcloud.size() << " points." << endl;
