...
#include <execution>    // prallel execution
...
void dotProductLock(const vector<int>& v0, const vector<int>& v1,
    const unsigned i_start, const unsigned i_end, unsigned long long& sum)
{
    //std::scoped_lock lock(mtx); // becomes faster, but sequential (not multi-threading anymore)
    for (unsigned i = i_start; i < i_end; i++)
    {
...
int main()
{
    ...
    cout << "std::inner_product (baseline)" << endl;
    {
    ...
    cout << "Naive (wrong calculation)" << endl;
    {
        const auto sta = chrono::steady_clock::now();
